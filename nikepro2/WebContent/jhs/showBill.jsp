...
<%
	...
	int origin_price_ho = (int) request.getAttribute("origin_price") + 3000;
	int result_price_ho = (int) request.getAttribute("result_price") + 3000;
	int price =0;
...
<!-- 				<div class="pay-side__info"> -->
	...
	<%	}
				...
				<!--  -->
						<span>상품 금액</span>
						<span id="origin_price"><%=origin_price_ho %> 원</span>
						<span>마일리지</span>
						<span>-<%=origin_price_ho-result_price_ho %>M</span>
						<span>주문합계</span>
						<span id="result_price"><%=result_price_ho %> 원</span>
				</div>
			</div>
		</main>
	...
	<script>
alert(origin_price_ho);
alert(result_price_ho);

/* let target_origin = document.getElementById("origin_price");
let target_result = document.getElementById("result_price");
target_origin.innerHTML = origin_price;
target_result.innerHTML = result_price; */
